...
public class ClassDAO {
	...
	/**
	 * 모임 테이블에 새로운 행 생성 (PK 값은 Sequence를 이용하여 자동 생성)
	 */
	public Class create(Class cl) throws SQLException {
		String sql = "INSERT INTO Class VALUES (clId_seq.nextval, ?, ?, ?, ?, SYSDATE, ?, ?, ?)";		
		Object[] param = new Object[] {cl.getLocation(), cl.getTitle(), cl.getPhoto(), cl.getContent(),
			cl.getUploader(), cl.getClass_like(), cl.getClass_view()};				
		...
		try {    
			jdbcUtil.executeUpdate(key);  // insert 문 실행
		   	...
		   	if(rs.next()) {
		   		...
		   		cl.setId(generatedKey); 	// id필드에 저장  
		   	}
		   	return cl;
		} catch (Exception ex) {
			jdbcUtil.rollback();
	...
	/**
	 * 주어진 ID에 해당하는 모임 정보를 삭제.
	 */
	public int remove(String clId) throws SQLException {
		String sql = "DELETE FROM Class WHERE c_id=?";		
		jdbcUtil.setSqlAndParameters(sql, new Object[] {clId});	// JDBCUtil에 delete문과 매개 변수 설정
	...
	public Class findClass(String keyword) throws SQLException {
		...
		jdbcUtil.setSqlAndParameters(sql, new Object[] {keyword, keyword});	// JDBCUtil에 query문과 매개 변수 설정
		Class cl = null;
		try {
	...
	/**
	 * 전체 모임 정보를 검색하여(최신순) List에 저장 및 반환
	 */
	public List<Class> findClassList() throws SQLException {
		...
		try {
			ResultSet rs = jdbcUtil.executeQuery();			// query 실행			
			List<Class> clList = new ArrayList<Class>();	// Class들의 리스트 생성
			while (rs.next()) {
				Class cl = new Class(			// Class 객체를 생성하여 현재 행의 정보를 저장
						...
						rs.getInt("class_like"),
						rs.getInt("class_view")	);
				clList.add(cl);				// List에 Class 객체 저장
			}		
			return clList;					
	...
	/**
	 * 신청 테이블 새로운 행 생성 (PK 값은 Sequence를 이용하여 자동 생성)
	 */
	public Apply create (Apply app) throws SQLException {
		...
		try {    
			jdbcUtil.executeUpdate(key);  // insert 문 실행
		   	...
		   	if(rs.next()) {
		   		int generatedKey = rs.getInt(1);   // 생성된 PK 값
		   		app.setId(generatedKey); 	// id필드에 저장  
		   	}
		   	return app;
		} catch (Exception ex) {
			jdbcUtil.rollback();
	...
	/**
	 * 주어진 ID에 해당하는 모임 신청 정보를 삭제.
	 */
	public int cancel(String appNum) throws SQLException {
		String sql = "DELETE FROM Apply WHERE appNum=?";		
		jdbcUtil.setSqlAndParameters(sql, new Object[] {appNum});	// JDBCUtil에 delete문과 매개 변수 설정
